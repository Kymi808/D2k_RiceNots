#!/bin/bash
# ============================================================
# SLURM job: normalized qw Transformer-MoE, no physics/reconstruction
# Override example:
#   TRAIN_FRAC=0.60 VAL_FRAC=0.20 MOE_NUM_EXPERTS=4 sbatch slurm_norm_qw_transformer_moe_nophys_norecon.sh
# ============================================================
#SBATCH --job-name=nqw-trmoe
...
#SBATCH --output=logs/norm_qw_transformer_moe_nophys_norecon_%j.out
#SBATCH --error=logs/norm_qw_transformer_moe_nophys_norecon_%j.err
...
echo "============================================"
echo "Job ID: $SLURM_JOB_ID  [NORM QW / TRANSFORMER-MOE / NO PHYS / NO RECON]"
echo "Split:   train=$TRAIN_FRAC val=$VAL_FRAC seed=$SPLIT_SEED"
echo "Heads:   $N_HEADS"
echo "FFN:     $TRANSFORMER_FFN_DIM"
echo "Experts: $MOE_NUM_EXPERTS top_k=$MOE_TOP_K"
echo "qw w:    $QW_WEIGHT"
echo "Date:    $(date)"
echo "============================================"

RUN_DIR=results/norm_qw_transformer_moe_nophys_norecon_${SPLIT_LABEL}_${SLURM_JOB_ID}
CKPT_DIR=checkpoints/norm_qw_transformer_moe_nophys_norecon_${SPLIT_LABEL}_${SLURM_JOB_ID}
